...
final class CodexServiceTierTests: XCTestCase {
    ...
    func testUnsupportedServiceTierDisablesFutureRetriesAndShowsUpdatePromptOnce() async throws {
        ...
        XCTAssertEqual(service.bridgeUpdatePrompt?.title, "Update Gogodex on your computer to use Speed controls")
        XCTAssertEqual(
            service.bridgeUpdatePrompt?.message,
            "This computer bridge does not support the selected speed setting yet. Update the Gogodex npm package to use Fast Mode and other speed controls."
        )
