...
- relay 域名目前使用 `codex.gotradetalk.com`；VPS 版基础健康检查、WebSocket 握手、bridge 重启恢复、relay 服务重启恢复已通过。正式上架目标是不运维 VPS，已新增 Cloudflare Workers + Durable Objects relay PoC，下一步需要部署到 Cloudflare 并让 TestFlight 包切过去复测。
