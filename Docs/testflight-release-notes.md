...
- relay 域名目前使用 `codex.gotradetalk.com`；VPS 版基础健康检查、WebSocket 握手、bridge 重启恢复、relay 服务重启恢复已通过。正式上架目标是不运维 VPS，Cloudflare Workers + Durable Objects relay PoC 已部署到 `workers.dev` 并通过线上 smoke test。下一步需要让 Mac bridge 和 TestFlight 包切到 Cloudflare relay 复测。
